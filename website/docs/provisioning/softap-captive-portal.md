...
Enable the Web UI with `CONFIG_WIFI_CFG_ENABLE_WEBUI=y` in your sdkconfig. By default it is the library's embedded Preact UI; your own frontend can take its place, embedded in your firmware or on a filesystem partition — see [Custom Web UI](../guides/custom-webui). The bundled Web UI is a Preact-based responsive interface (~16KB gzipped, with a language selector for English, German, Spanish, French and Vietnamese) that provides:
